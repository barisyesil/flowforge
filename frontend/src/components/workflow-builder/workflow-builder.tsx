...
import { Inspector } from "@/components/workflow-builder/inspector";
...
/** Görsel süreç tasarımcısı: [ Canvas (node graph) ] [ Inspector ]. */
export function WorkflowBuilder() {
  ...
  async function handleSave() {
    ...
    if (saved) {
      loadWorkflow(saved);
      toast.success(t("wf.saved", { name: saved.name, version: saved.version }));
    } else {
  ...
  return (
    <div className="flex h-[calc(100dvh-9rem)] min-h-[520px] flex-col gap-4">
      ...
      <ReactFlowProvider>
        <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[1fr_320px]">
          <WorkflowCanvas />
          <div className="hidden h-full overflow-y-auto rounded-xl border bg-card p-4 lg:block">
            <Inspector />
          </div>
        </div>
      </ReactFlowProvider>
